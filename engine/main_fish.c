...
struct AnimParam anim_params[11] = {
    ...
    {18.2, 0.0, 0}, // ltr-fast (same leisurely pace as the tank)
    {18.2, 0.0, 1}, // rtl-fast
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *window = SDL_CreateWindow("Aquarium", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800, 600, SDL_WINDOW_SHOWN | SDL_WINDOW_BORDERLESS | SDL_WINDOW_RESIZABLE);
    if (window == NULL) {
    ...
    while (!quit) {
        ...
        for (size_t i = 0; i < entity_count; i++) {
            ...
            float flap_period = 1.15f / speed_mult;
            if (flap_period < 0.16f) flap_period = 0.16f;
